...
**Important** if you use the flag `-B`, or one with the type `-b**` below (_any flag except_ `-bc` _and_ `-bm`) it **must** be followed by either a passage query, or a bookmark name, respectively.
